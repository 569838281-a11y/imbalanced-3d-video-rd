# Stage-3b local: Macula Detached → TD vs Bilateral (UNet3D 50ep, resume + early stop)
$ErrorActionPreference = "Continue"
$repo = "C:\Users\ADMIN\Desktop\test_5"
$ckptDir = "D:\ERDES\checkpoints\stage3_detached_td_vs_bilateral_50ep_local"
$py = "D:\anaconda3\envs\erdes\python.exe"
$logFile = "D:\ERDES\logs\stage3_detached_td_vs_bilateral_train.log"

New-Item -ItemType Directory -Force -Path $ckptDir, "D:\ERDES\logs", "D:\ERDES\temp" | Out-Null

if (-not (Test-Path "$repo\data\splits\macula_detached_td_vs_bilateral\train.csv")) {
...
Write-Host "Stage-3b Macula_Detached: TD vs Bilateral | ckpts -> $ckptDir"
Write-Host "WARN: single-patient-per-class in current data — prototype metrics only."

& $py -u erdes/train.py `
  experiment=macula_detached_td_vs_bilateral/unet3d_50ep_local `
  $ckptArg `
  extras.print_config=false `
  test=false `
  'callbacks.model_summary=null' `
  tags=[erdes,unet3d,stage3,detached,td_vs_bilateral,50ep,local] `
